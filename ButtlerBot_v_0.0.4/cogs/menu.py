...
class HelpCommands(commands.Cog):
    ...
    async def paginate(self, ctx):
        ...
        embed4 = discord.Embed(timestamp = now, color=ctx.author.color).add_field(name=f"How To Present Your Code In This Discord:",
                                                                value=f"If you're unsure of how to paste code within {ctx.guild.name}, then type `/buttlerpaste` to get the insider scoop :smile:")
        ...
        embed6 = discord.Embed(timestamp = now, color=ctx.author.color).add_field(name=f"Staff Applications:",
                                                                value="If you would like to apply to become a staff member, please type `/buttlerstaffapp` to receive the application within the #staff_applications channel")
        ...
        embed7 = discord.Embed(timestamp = now, color=ctx.author.color).add_field(name=f"Bot Updates",
                                                                value="For a list of the most recent bot updates, please type `/buttlerbotupdates` to get the link to our text channel for our updates that come out for our communities bots.")
        ...
        embed8 = discord.Embed(timestamp = now, color=ctx.author.color).add_field(name=f"List Of Bots In {ctx.guild.name}",
                                                                value="To see a list of the bots we have, please type `/buttlerbotlist`")
        ...
        embed10 = discord.Embed(timestamp = now, color=ctx.author.color).add_field(name=f"Need Help?",
                                                                value=f"Do you need help with your code? Then type `/buttlerhelplanguage` to get help with your code! Be sure to replace `<languange>` with the language of the code you're needing assistance with! **Be advised, this discord only provides support for Java, C#, Python, JavaScript, and HTML!**")
        ...
        embed14 = discord.Embed(timestamp = now, color=ctx.author.color).add_field(name=f"{self.bot.user.name} not responding?",
                                                                value="type `/buttlerping` to get a pong!")
        embed14.set_image(url=x["results"][y15]["image"])
        embed14.set_footer(text="Page 14/19")
        embed14.timestamp = datetime.datetime.now()
        embed15 = discord.Embed(timestamp = now, color=ctx.author.color).add_field(name=f"Nick Names!",
        ...
        embed16 = discord.Embed(timestamp = now, color=ctx.author.color).add_field(name=f"**Submitting Your Code To A Team?**",
                                                                value="To submit your code to the team submissions channel, type `/buttlerteamsubmit <link to your pastebin>`")
        embed16.set_image(url=x["results"][y17]["image"])
        embed16.set_footer(text="Page 16/19")
        embed16.timestamp = datetime.datetime.now()
        embed17 = discord.Embed(timestamp = now, color=ctx.author.color).add_field(name=f"**Submitting A Discord Suggestion?**",
                                                                value=f"To submit your discord suggestion, type `/buttlerdiscordsubmit <your_suggestion>`")
        embed17.set_image(url=x["results"][y18]["image"])
        embed17.set_footer(text="Page 17/19")
        embed17.timestamp = datetime.datetime.now()
        embed18 = discord.Embed(timestamp = now, color=ctx.author.color).add_field(name=f"**Submitting A Suggestion For A Future Bot?**",
                                                                value=f"Want to submit an idea for a future bot? type `/buttlerbotsubmit <your_suggestion>`")
        embed18.set_image(url=x["results"][y19]["image"])
        embed18.set_footer(text="Page 18/19")
        embed18.timestamp = datetime.datetime.now()
        embed19 = discord.Embed(timestamp = now, color=ctx.author.color).add_field(name=f"Disclaimer!",
