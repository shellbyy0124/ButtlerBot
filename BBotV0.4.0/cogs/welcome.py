...
class WelcomingMembers(commands.Cog):
    ...
    async def on_member_join(self, member:discord.Member):
        ...
        welcome = discord.Embed(color=self.color, timestamp=self.timestamp, title=f"Hi and Welcome!", description=f"My name is {self.bot.user.name}, and I will be your virtual assistant during your stay here at the Learning Together Discord Community! :smile:", inline=False)
        ...
        if msg.content.lower() == "!continue":
            ...
            if ans1.content.lower() == "ready":
                ...
                if all(i.isprintable() for i in ans2.content):
                    ...
                    embed3 = discord.Embed(color=self.color, timestamp=self.time, title="ButtlerBot Profile Creator", description="What is your favorite programming language? If you have multiple, please separate them using commas `,`\nIf not, then enter N/A. No blank answers will be accepted", inline=False).set_thumbnail(url=self.url).set_footer(text="You have 60 seconds to respond, or this command will time out, and you will need to get with a staff member in the #new_member_support channel")
                    await msg1.edit(embed=embed3)
                    ans3 = await self.bot.wait_for('message', timeout=60)

                    if all(i.isprintable() for i in ans3.content):

                        embed4 = discord.Embed(color=self.color, timestamp=self.time, title="ButtlerBot Profile Creator", description="What is your preferred programming language? If you have multiple, please separate them using a comma `,`\nIf not, then enter N/A. No blank answers will be accepted", inline=False).set_thumbnail(url=self.url).set_footer(text="You have 60 seconds to respond, or this command will timeout, and you will need to get with a staff member in the #new_member_support channel")
                        await msg1.edit(embed=embed4)
                        ans4 = await self.bot.wait_for('message', timeout=60)

                        if all(i.isprintable() for i in ans4.content):
                            
                            embed5 = discord.Embed(color=self.color, timestamp=self.time, title="ButtlerBot Profile Creator", description="What other languages do you know? If you have multiple, please separate them using a comma `,`\nIf not, then enter N/A. No blank answers will be accepted", inline=False).set_thumbnail(url=self.url).set_footer(text="You have 60 seconds to respond, or this command will timeout, and you will need to get with a staff member in the #new_member_support channel")
                            await msg1.edit(embed=embed5)
                            ans5 = await self.bot.wait_for('message', timeout=60)

                            if all(i.isprintable() for i in ans5.content):
                                
                                embed6 = discord.Embed(color=self.color, timestamp=self.time, title="ButtlerBot Profile Creator", description="Do you have any certifications that you would like to list? If yes, separate them by commas `,`\nIf not, then enter N/A. No blank answers will be accepted", inline=False).set_thumbail(url=self.url).set_footer(text="You have 60 seconds to respond, or this command will timeout, and you will need to get with a staff member in the #new_member_support channel")
                                await msg1.edit(embed=embed6)
                                ans6 = await self.bot.wait_for('message', timeout=60)

                                if all(i.isprintable() for i in ans6.content):

                                    embed7 = discord.Embed(color=self.color, timestamp=self.time, title="ButtlerBot Profile Creator", description="Do you have any awards that you would like to list? if yes, separate them by commas `,`\nIf not, then enter N/A. No blank answers will be accepted", inline=False).set_thumbnail(url=self.url).set_footer(text="You have 60 seconds to respond, or this command will timeout, and you will need to get with a staff member in the #new_member_support channel")
                                    await msg1.edit(embed=embed7)
                                    ans7 = await self.bot.wait_for('message', timeout=60)

                                    if all(i.isprintable() for i in ans7.content):

                                        embed8 = discord.Embed(color=self.color, timestamp=self.time, title="ButtlerBot Profile Creator", description="Do you have any degrees that you would like to list? if yes, separate them by commas `,`\nIf not, then enter N/A. No blank answers will be acceptaed", inline=False).set_thumbnail(url=self.url).set_footer(text="You have 60 seconds to respond, or this command will timeout, and you will need to get with a staff member in the #new_member_support channel")
                                        await msg1.edit(embed=embed8)
                                        ans8 = await self.bot.wait_for('message', timeout=60)

                                        if all(i.isprintable() for i in ans8.content):

                                            final_embed = discord.Embed(color=self.color, timestamp=self.time, title=f"Profile For: {member.display_name}", description=f"""Programming For: {ans2.content}
                                                                                                                                                                            ...
                                                                                                                                                                            Account Balance: {starting_balance}""").set_thumbnail(url=self.url).set_footer(text="If you are satisfied with your profile, please type `!save`, otherwise, you will need to go to the #new_member_support channel")
                                            await msg1.edit(embed=final_embed)
                                            ...
                                            if ans9.content.lower() == "!save":
                                                ...
                                                await channel.send(f"User, {member.name}, has agreed to the rules on {self.time}")
                                                await channel1.send(final_embed)
                                                await msg1.delete()
                                                await ans1.delete()
                                                await ans2.delete()
                                                await ans3.delete()
                                                ...
                                                for x in answers:
                                                    new_ans.append(', '.join(x))
                                                info = data["users"][str(member.name)][{"id" : int(member.id), "bank" : int(starting_balance), "roles" : str(member.roles), "information" : str(new_ans)}]

                                                with open('./users.json', 'w', encoding='utf-8-sig') as g:
                                                    data = json.dump(data, info, indent=4)
                                                    
        elif msg.content.lower() == "!noprofile":

            embed1 = discord.Embed(color=self.color, timestamp=self.time, title="Hello, and Welcome!", description=f"Welcome to {LT.name}!").set_thumbnail(url=self.url)
            msg1 = await member.send(embed=embed1)
            ...
            role = self.bot.get_role("Dev")
            role1 = self.bot.get_role("Head Dev")

            error = discord.Embed(color=self.color, timestamp=self.time, title=":red_circle: **__ERROR:__**:red_circle:", description=f"Attention {role1.mention} and {role.mention}. The error is coming from welcome.py. This error message is at line 114")
            msg1 = await member.send("An error message has been sent to the dev team. Please go to the #new_member_support channel for further assistance")
            channel = self.bot.get_channel(errors)
            a = await channel.send(embed=error)
            await a.pin()
